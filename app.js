...
function renderChatList() {
    ...
    chatList.innerHTML = sortedChats.map(chat => `
        <div class="group flex items-center gap-2 p-3 rounded-xl cursor-pointer transition-colors ${chat.id === activeChatId ? 'bg-brand-accent/15 border border-brand-accent/25' : 'hover:bg-brand-surface'}"
...
function renderChat() {
    ...
    chatContainer.innerHTML = `
        <div class="flex gap-3 message-enter">
            <div class="w-8 h-8 bg-gradient-to-br from-brand-accent to-brand-accentDeep rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold">
                J
            </div>
...
async function addMessageToUI(role, content, isError = false, withTypingEffect = false) {
    ...
    if (role === 'user') {
        messageDiv.innerHTML = `
            <div class="ml-auto bg-brand-accent/15 border border-brand-accent/25 rounded-2xl rounded-tr-sm px-4 py-3 max-w-[85%]">
                <div class="text-gray-200">${escapeHtml(content)}</div>
            </div>
            <div class="w-8 h-8 bg-brand-surface rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold text-brand-accent">
                U
            </div>
        `;
        ...
        avatar.className = 'w-8 h-8 bg-gradient-to-br from-brand-accent to-brand-accentDeep rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold';
